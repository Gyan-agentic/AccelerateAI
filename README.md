...
Upload raw CSV files via the Streamlit UI
Type a business question (e.g. "Which product category had the highest sales in Q4?")
Review and approve AI-generated transformation rules at three checkpoints
Receive a complete HTML executive report with SQL-backed charts
What the agents do:
...
THINK — reads the goal from the Supervisor and understands the context
INSPECT — calls its inspect tool to preview inputs before touching any data
PLAN — states an explicit plan based on what it observed
ACT — calls its execution tool to process data
VERIFY — confirms outputs and reports back to the Supervisor
...
Step-by-step usage
Upload CSV files — drag and drop your raw data files
Enter your business question — plain English, e.g. "Which store had the highest revenue last month?"
Click Run Phase 1 — agents profile data and generate Bronze STTM
Review Bronze STTM — check the transformation rules, click Approve
Click Run Phase 2 — Bronze agent ingests data, Silver STTM generated
Review Silver STTM — check cleansing rules, click Approve
Click Run Phase 3 — Silver agent cleanses data, Gold STTM generated
Review Gold STTM — check analytics table structure, click Approve
Click Run Phase 4 — Gold tables materialised, report generated
View report — executive summary with charts answers your question
